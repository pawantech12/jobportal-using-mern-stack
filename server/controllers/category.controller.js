    ...
    if (req.file) {
      ...
      const uploadResult = await new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          { folder: "job_board_website/category_images" },
          (error, result) => {
            if (error) {
    ...
    const categoriesWithPostCount = await Promise.all(
      categories.map(async (category) => {
        const jobPostCount = await JobPost.countDocuments({
          jobCategory: category._id, // Count blog posts where the category matches
        });

        return {
          ...category._doc, // Spread category details
          jobPostCount, // Add blog post count to the category object
        };
      })
    );
...
const getAllJobsByCategoryId = async (req, res) => {
  try {
    const categoryId = req.params.id;
    const jobs = await JobPost.find({ category: categoryId })
      .populate({
        path: "employerId",
        populate: {
          path: "userId",
